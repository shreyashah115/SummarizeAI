"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.HighlightModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const highlight_schema_1 = require("./model/highlight.schema");
const highlight_controller_1 = require("./controller/highlight.controller");
const highlight_service_1 = require("./service/highlight.service");
const openai_controller_1 = require("./controller/openai.controller");
const openai_service_1 = require("./service/openai.service");
let HighlightModule = class HighlightModule {
};
HighlightModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forRoot('mongodb://localhost:27017/Stream'),
            mongoose_1.MongooseModule.forFeature([{ name: 'Highlight', schema: highlight_schema_1.HighlightSchema }]),
        ],
        controllers: [highlight_controller_1.HighlightController, openai_controller_1.OpenAIController],
        providers: [highlight_service_1.HighlightService, openai_service_1.OpenAIService],
    })
], HighlightModule);
exports.HighlightModule = HighlightModule;
//# sourceMappingURL=app.module.js.map