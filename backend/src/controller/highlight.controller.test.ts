...
import { HighlightDocument } from '../model/highlight.schema';
...
describe('HighlightController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    highlightModel = moduleFixture.get<Model<HighlightDocument>>(
      getModelToken('Highlight'),
    );
  });
  ...
  describe('/api/v1/highlight (GET)', () => {
    it('should return all highlights', async () => {
      // Create test highlights in the database
